    ...
    <div className="mb-12 grid gap-10 md:grid-cols-2">
      <div>
        {mainImage ? (
          ...
          <div className="flex h-80 w-full items-center justify-center rounded-xl border border-line bg-background text-muted">
            No image
          </div>
        )}
      ...
      <div>
        <h1 className="mb-2 font-display text-3xl font-semibold text-foreground">{product.name}</h1>

        {price != null && (
          <div className="mb-1 flex items-baseline gap-3">
            <p className="text-2xl font-bold text-deep">{formatPrice(price)}</p>
            {hasDiscount && (
        ...
        {groups.map((group) => (
          <div key={group.attribute.documentId} className="mb-4">
            <p className="mb-2 text-sm font-medium text-foreground">{group.attribute.name}</p>
        ...
        <div className="mt-6 flex items-center gap-4">
          <div className="flex items-center rounded-md border border-line">
            <button
              ...
              className="px-3 py-2 text-foreground transition hover:text-brand"
              aria-label="Decrease quantity"
            >
              −
            </button>
            <span className="min-w-8 px-2 text-center text-sm text-foreground">{quantity}</span>
            <button
              onClick={() =>
                setQuantity((q) =>
